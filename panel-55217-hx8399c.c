...
static inline int hx8399c_55217_dsi_write(struct hx8399c_55217 *_hx8399c_55217,
  ...
  /*return mipi_dsi_dcs_write_buffer(_hx8399c_55217->dsi, seq, len);*/
    ...
    addr = (char*)seq;
    if ((int)*addr < 0xB0)
        ret = mipi_dsi_dcs_write_buffer(_hx8399c_55217->dsi, seq, len);
    else
        ret = mipi_dsi_generic_write(_hx8399c_55217->dsi, seq, len);
    if (ret < 0) {
        dev_err(&_hx8399c_55217->dsi->dev, "error %zd writing seq: %ph\n", ret, seq);
    }
    return ret;
}
...
static int hx8399c_55217_prepare(struct drm_panel *panel) {
    ...
    /*const struct hx8399c_55217_panel_desc *desc = of_device_get_match_data(&hx8399c_55217->dsi->dev);*/
...
static int hx8399c_55217_enable(struct drm_panel *panel) {
    ...
    /*
     *if (hx8399c_55217->backlight) {
     *    hx8399c_55217->backlight->props.power = FB_BLANK_UNBLANK;
     *    backlight_update_status(hx8399c_55217->backlight);
     *}
     *backlight_enable(hx8399c_55217->backlight);
     */
...
static int hx8399c_55217_disable(struct drm_panel *panel) {
    ...
    /*
     *if (hx8399c_55217->backlight) {
     *    hx8399c_55217->backlight->props.power = FB_BLANK_POWERDOWN;
     *    backlight_update_status(hx8399c_55217->backlight);
     *}
     *backlight_disable(hx8399c_55217->backlight);
     */
...
static int hx8399c_55217_dsi_probe(struct mipi_dsi_device *dsi) {
    ...
    hx8399c_55217->panel.prepare_upstream_first = true;
 ...
 *    if (backlight) {
 *        hx8399c_55217->backlight = of_find_backlight_by_node(backlight);
